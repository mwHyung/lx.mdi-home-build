...
const SearchTab: FC<Props> = ({ tabsList }) => {
  ...
  return (
    <div className="py-32 bg-pub-bg">
      <ul className="flex flex-wrap max-w-[1680px] m-[0_auto] px-48">
        {tabsList.map((list, idx) => (
          ...
          >
            <span className="text-[2rem] font-medium leading-[2rem]">{list.name}</span>
            <span className="text-[1.4rem] font-bold leading-[1.4rem]">{list.hits}</span>
          </li>
        ))}
      </ul>
    </div>
